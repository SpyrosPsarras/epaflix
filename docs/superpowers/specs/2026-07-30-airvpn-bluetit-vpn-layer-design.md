...
- **Credentials.** Bluetit's recommended-server selection needs the AirVPN **account username and
  ...
  risk; they live in a SOPS Secret. In the credential store
  (`.github/instructions/secrets.enc.yaml`) as `airvpn_user` / `airvpn_password`. Those are
  top-level keys sharing the `airvpn_*` prefix, not a nested block, so the read is
  `sops -d --extract '["airvpn_user"]' .github/instructions/secrets.enc.yaml`. (They were
  briefly added as `airVPN_user` / `airVPN_password`; the owner renamed them to match the other
  `airvpn_*` keys on 2026-07-30, mid-implementation.)
