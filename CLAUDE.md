...
- **A check that cannot fail is not evidence.** Before citing a command as proof, confirm it was capable of returning the other answer. Verification failures in this repo have consistently failed toward "nothing here" rather than toward an error: an empty `origin/main..HEAD` range reporting a file unchanged, a render without the age key reporting manifests absent, a basic-regex `grep "a|b"` reporting a clean tree. Known traps and their fixes: `docs/verification-traps.md` (#1096).
