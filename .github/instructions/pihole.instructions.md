...
| `ntfy.epaflix.com` | **192.168.10.102** | ntfy - Traefik `internal` entry point, not the public LB (#904). Live since PR #1088's deploy gate: the `address=` line is in `10-epaflix.conf` on the box and the LAN resolver answers `.102`. Load-bearing since #920 - Proxmox VE (topic `pve-backups`, `1-proxmox/pbs/notifications.cfg`) and the TrueNAS GPU cron (`truenas-alerts`) publish through this hostname, and the plaintext `192.168.10.112:8091` LoadBalancer they used to dial is retired, so if this record goes both of them go silent. The `ntfy-internal` IngressRoute carries **no** Authentik middleware on purpose: those publishers are machines that POST with no interactive login. There is no `websecure` route for this name at all - a public one would publish an unauthenticated publish/subscribe endpoint |
