...
Keep LAN records in `dnsmasq.d/`, not in the Pi-hole API (`dns.hosts`, `misc.dnsmasq_lines` in `/etc/pihole/pihole.toml`): API entries are not in git, and a `dns.hosts` record silently overrides a tracked `address=` line.
