...
try {
  ...
  for (const id of ["claude-fable-5-1", "gpt-6-astra"]) {
    assert.deepEqual(models[id].modalities, { input: ["text", "image"], output: ["text"] })
    assert.equal(models[id].attachment, true)
  }
  for (const id of ["gpt-5.3-codex-spark", "or-glm-5.3-flash"]) {
    assert.deepEqual(models[id].modalities, { input: ["text"], output: ["text"] })
    assert.equal(models[id].attachment, false)
  }
