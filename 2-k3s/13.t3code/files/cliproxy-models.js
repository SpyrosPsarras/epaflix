  ...
  async config(config) {
    ...
    try {
      ...
      for (const model of catalog.data) {
        ...
        } else if (owner === "anthropic") {
          name = `anthropic-${id.replace(/(\d)-(\d)(?=-|$)/g, "$1.$2")}`
          npm = "@ai-sdk/anthropic"
        } else if (owner === "openai" && !id.startsWith("gpt-image-")) {
      ...
      if (cached?.url !== url || !cached.models || !Object.keys(cached.models).length) throw error
    ...
    await writeFile(temporary, JSON.stringify({ url, updatedAt: new Date().toISOString(), models }), { mode: 0o600 })
