...
- [ ] **Post-upgrade apply confirmation:** the `iac-service-account` BlueprintInstance reports `status: successful`; if it shows 403 / not-applied, trigger blueprint discovery or `kubectl -n app-authentik rollout restart deploy/authentik-worker` (the #185 worker-discovery gotcha). Confirm the `ak-iac` user + token are present via `GET /api/v3/core/users/me/` → 200.
...
```bash
kubectl -n app-authentik exec deploy/authentik-worker -- \
