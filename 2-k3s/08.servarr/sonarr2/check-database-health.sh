#!/bin/bash
# Sonarr2 Database Health Check Script
...
echo -e "${BLUE}=========================================="
echo "Sonarr2 Database Health Check"
echo -e "==========================================${NC}"
echo ""
...
DB_NAME="sonarr2-main"
DB_USER="sonarr2"

if [ -z "$SONARR2_PW" ]; then
...
run_query() {
  PGPASSWORD="${SONARR2_PW}" psql -h "${DB_HOST}" -U "${DB_USER}" -d "${DB_NAME}" -t -c "$1"
}
...
if [ $DUPLICATES_FOUND -eq 0 ] && [ $SEQUENCES_BAD -eq 0 ]; then
  ...
  if [ $DUPLICATES_FOUND -eq 1 ]; then
    ...
    echo "  kubectl apply -f /home/spy/Documents/Epaflix/k3s-proxmox/2-k3s/08.servarr/sonarr2/fix-duplicate-series.yaml"
    echo "  kubectl apply -f /home/spy/Documents/Epaflix/k3s-proxmox/2-k3s/08.servarr/sonarr2/fix-duplicate-episodes.yaml"
    echo "  kubectl apply -f /home/spy/Documents/Epaflix/k3s-proxmox/2-k3s/08.servarr/sonarr2/fix-duplicate-episodefiles.yaml"
    echo ""
  fi
  ...
  echo -e "${YELLOW}After fixes, restart Sonarr2:${NC}"
